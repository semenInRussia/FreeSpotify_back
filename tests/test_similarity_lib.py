...
def test_search_string_similar_to():
    actual = similarity_lib.search_string_similar_to(
        "acdc", ["metallica", "Black Sabbath", "AC/DC"]
    )
...
def test_filter_and_sort_strings_by_min_similarity_to():
    actual = list(
        similarity_lib.filter_and_sort_strings_by_min_similarity_to(
            "AC/DC", ["a", "ac dc", "b", "AC/DC", "AC/dc"]
        )
    )
...
def test_filter_objects_by_min_similarity_to():
    actual = list(
        similarity_lib.filter_objects_by_min_similarity_to(
...
def test_sort_objects_by_similarity_to():
    actual = list(
        similarity_lib.sort_objects_by_similarity_to(
            "AC/DC", ["AC DC", "AC/DC", "ac dc", "a", "acdc"]
        )
    )
