...
def select_elements_on_page(url: str,
                            css_selector: str,
                            method_name: str='get',
                            **kwargs) -> list[Tag]:
    soup = get_bs(url, method_name, **kwargs)
...
@cached_function
def cached_select_elements_on_page(url: str,
                                   css_selector: str,
                                   method_name: str='get',
                                   **kwargs) -> list[Tag]:
...
def cached_select_one_element_on_page(url: str,
                                      css_selector: str,
                                      method_name: str='get',
                                      **kwargs):
    soup = cached_get_bs(url, method_name, **kwargs)
...
def get_first_link_by_elements_or_raise_exception(elements: list[Tag],
    ...
    try:
        ...
        return get_absolute_url_by_element(element,
                                           base_url,
                                           url_attribute_of_tag)
...
@cached_function
def cached_search_on_page(url: str,
                          pattern: str,
                          method: str = 'get',
                          **kwargs) -> list[str]:
